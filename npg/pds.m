function out = pds(NLL,proximal,xInit,opt)
...
y=zeros(size(x));
proximal.op=@(x,t) max(min(x,abs(t)),-abs(t));
sigma=(sqrt(opt.Lip^2/16+1)-opt.Lip/4)*1;
tau=1/(sigma+opt.Lip/2) * 1;
rho=(2-opt.Lip/2/(1/tau-sigma)) * 1;
...
while(true)
    ...
    [oldCost,grad] = NLL(x);
    ybar=proximal.op(y+sigma*Psit(alpha),u);
    xbar=opt.prj_C(alpha-tau*(grad+Psi(2*ybar-y)));
    newX=alpha+rho*(xbar-alpha);
    y = y+rho*(ybar-y);
