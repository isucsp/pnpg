...
%                               
%   'Interpolation'              String specifying the type of interpolation used
%                                between the fan-beam and parallel-beam data.
%                                
...
%                               
%   'ParallelCoverage'           String specifying the range of rotation
%                                angles of the parallel-beam projection data.
%                                
...
%   $Revision: 1.1.6.12.2.1 $  $Date: Fri 24 Jan 2014 10:06:45 AM CST
...
%args.RotationCenter=args.RotationCenter+1;
m = size(F,1);
...
gammaDeg  = formGammaVector(m,d,fanSpacing,fanSensorGeometry,args.PerpenCenter,args.RotationCenter);
...
%ploc = formPlocVector(d,gammaDeg,args.ParallelSensorSpacing,args.RotationCenter,args.PerpenCenter);
plocRange=([1,m]-args.RotationCenter)*fanSpacing*d./...
        sqrt((([1,m]-args.PerpenCenter)*fanSpacing).^2+d^2);
ploc = utils.formVectorCenteredOnZero(args.ParallelSensorSpacing,plocRange(1),plocRange(2));
%ploc = formPlocVector(d,gammaDeg,args.ParallelSensorSpacing,args.RotationCenter,args.PerpenCenter);
...
%------------------------------------------------------------------------------------
function P = fan2paraInterp(F,d,gammaDeg,thetaDeg,ploc,pthetaDeg,interp,isFanCoverageCycle,args)
...
if isFanCoverageCycle
    ...
    P = fan2paraInterp(Fpad,d,gammaDeg,thetapad,...
                       ploc,pthetaDeg,interp,false,args);
    ...
    t=((1:m)-args.RotationCenter)*args.FanSensorSpacing*d./...
        sqrt((((1:m)-args.PerpenCenter)*args.FanSensorSpacing).^2+d^2);
    for i=1:numelPtheta
...
%-------------------------------------------------------------------
function gammaDeg = formGammaVector(m,d,...
        fanSpacing,fanSensorGeometry,PerpenCenter,RotationCenter)
...
if strcmp(fanSensorGeometry,'line')
    ...
    g = ((1:m)-PerpenCenter)*fanSpacing;
    gammaDeg = atan(g/d)*180/pi;
    gammaDeg = gammaDeg - atan((RotationCenter-PerpenCenter)*fanSpacing/d)*180/pi;
...
narginchk(2,18);
%iptchecknargin(2,18,nargin,mfilename);
...
args.RotationCenter            = floor((size(F,1)+1)/2);
