...
%|
%| Caution: in most cases the "differences" here wrap around image edges.
...
for order=1:2
	...
	types = {'def', 'ind', 'mex', 'sparse', 'convn', 'imfilter'};
	for it=1:length(types)
		...
		switch C.arg.type_diff
		case {'convn', 'imfilter'} % trick: these require wt
			wt = Rweights(ig.mask, C.arg.offsets, ...
				'type_wt', 'array', ...
