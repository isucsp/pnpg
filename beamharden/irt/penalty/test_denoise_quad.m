...
% motivated by sense map issues with m. allison
...
if ~isvar('R'), printm 'R'
	...
	ig = image_geom('nx', 2^6, 'ny', 2^5-2, 'dx', 1);
...
%	R = Reg1(args{:}, 'type_penal', 'mat');
	psf = qpwls_psf(1., R, 1., ig.mask); % report expected blur (at image center)
end

f.niter = 250;
% todo: changes a lot with iteration so make a movie to watch it!

if 0 % run qpwls algorithm for regularized fitting
	A = 1;
	init = ig.zeros;
	yy = ig.unitv('c', [ig.nx/2-2 1]);
	xh = qpwls_pcg1(init(ig.mask), A, 1, ...
