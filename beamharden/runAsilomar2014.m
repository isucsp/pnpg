...
% vary the number of measurements, with continuation
if(any(runList==007))
%   filename = [mfilename '_007.mat']; if(~exist(filename,'file')) save(filename,'filename'); else load(filename); end; clear('opt'); opt.B=2^16; aa=-3:-0.1:-6;
%   filename = [mfilename '_027.mat']; if(~exist(filename,'file')) save(filename,'filename'); else load(filename); end; clear('opt'); opt.B=2^12; aa=-1:-0.2:-5;
    filename = [mfilename '_037.mat']; if(~exist(filename,'file')) save(filename,'filename'); else load(filename); end; clear('opt'); opt.B=2^9; aa=-0.2:-0.1:-4;
