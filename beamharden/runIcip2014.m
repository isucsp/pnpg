...
%   $Revision: 0.1 $ $Date: Thu 06 Feb 2014 01:35:58 AM CST
...
opt.rInit=5000;
opt.trueMu= conf.trueKappa;
opt.trueIe= conf.trueIe;
...
if(any(runList==20))
    ...
    out20{i}=beamhardenSpline(conf.PhiM,conf.PhiMt,...
...
if(any(runList==21)) % beamhardening with refinement
    ...
    for jj=7 %[5:6] %1:length(rCoeff)
        ...
        for i=1:length(aArray)
            opt.a=aArray(i);
            out{i}=beamhardenASSparseResampleMu(conf.PhiM,conf.PhiMt,...
                conf.Psi,conf.Psit,conf.y,initSig,opt);
