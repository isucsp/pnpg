...
% Figure 4:   Objective evolution versus iteration and compute time.
        ...
        load(filename,'*001'); i=1; j=1;
        initSig = finit; opt.maxItr=maxiter;
        conf.y=y; conf.Phi=A; opt.u=tau; conf.Phit=AT;
        conf.Psit=@(x) x; conf.Psi=@(x) x; opt.errorType=2;
        opt.noiseType='poisson'; opt.thresh=tolerance; opt.trueAlpha=f;

        out.alpha=fhatSPIRAL; out.p=iterationsSPIRAL; out.cost=objectiveSPIRAL;
        out.reconerror=reconerrorSPIRAL; out.time=cputimeSPIRAL;
        spiral001{i,j}=out;
        spiral001{i,j}.fVal(1)=0.5*norm(conf.Phi(spiral001{i,j}.alpha)-conf.y)^2;
        spiral001{i,j}.fVal(2)=norm(spiral001{i,j}.alpha.*(spiral001{i,j}.alpha<0))^2;
        spiral001{i,j}.fVal(3)=sum(abs(conf.Psit(spiral001{i,j}.alpha)));
...
%             reconstructions and the true phantom image. 
        ...
        if(0)
            A = @(x)  reshape(conv2(reshape(x,floor(sqrt(length(x(:)))),[]),blur,'same'),[],1);
            AT = @(x) reshape(conv2(reshape(x,floor(sqrt(length(x(:)))),[]),blur,'same'),[],1);
            W = @(x)  reshape(midwt(reshape(x,floor(sqrt(length(x(:)))),[]),wav),[],1);
            WT = @(x) reshape( mdwt(reshape(x,floor(sqrt(length(x(:)))),[]),wav),[],1);
            f=f(:); y=y(:);
        end
        ...
        load(filename,'*002'); i=1; j=1;
        initSig = finit; opt.maxItr=maxiter;
        conf.y=y; conf.Phi=A; opt.u=tauonb; conf.Phit=AT;
        conf.Psit=WT; conf.Psi=W; opt.errorType=2;
        opt.noiseType='poisson'; opt.thresh=tolerance; opt.trueAlpha=f;
        ...
        out.alpha=fhatSPIRALonb; out.p=iterationsSPIRALonb; out.cost=objectiveSPIRALonb;
        out.reconerror=reconerrorSPIRALonb; out.time=cputimeSPIRALonb;
        spiral002{i,j}=out;
        spiral002{i,j}.fVal(1)=0.5*norm(reshape(conf.Phi(spiral002{i,j}.alpha)-conf.y,[],1))^2;
        spiral002{i,j}.fVal(2)=norm(spiral002{i,j}.alpha(:).*(spiral002{i,j}.alpha(:)<0))^2;
        spiral002{i,j}.fVal(3)=sum(sum(abs(conf.Psit(spiral002{i,j}.alpha))));
        ...
        subplot(2,1,1)
            ...
            hold on; plot(0:npg002{1}.p-1,npg002{1}.RMSE,'k');
            legend('ONB','TV','RDP','RDP-TI','NPG')
        ...
        subplot(2,1,2)
            ...
            hold on; plot(npg002{1}.time,npg002{1}.RMSE,'k');
            legend('ONB','TV','RDP','RDP-TI','NPG')
...
%             reconstructions and the true phantom image. 
       ...
       % return
        ...
        out.reconerror=reconerrorSPIRALonb; out.time=cputimeSPIRALonb;
        spiral003{i,j}=out;
        spiral003{i,j}.fVal(1)=0.5*norm(reshape(conf.Phi(spiral003{i,j}.alpha)-conf.y,[],1))^2;
        spiral003{i,j}.fVal(2)=norm(spiral003{i,j}.alpha(:).*(spiral003{i,j}.alpha(:)<0))^2;
        spiral003{i,j}.fVal(3)=sum(sum(abs(conf.Psit(spiral003{i,j}.alpha))));
        ...
        subplot(2,1,1)
            ...
            plot(0:iterationsSPIRALrdp,reconerrorSPIRALrdp,'m')
            plot(0:iterationsSPIRALrdpti,reconerrorSPIRALrdpti,'g')
            hold on; plot(0:npg003{1}.p-1,npg003{1}.RMSE,'k');
            legend('ONB','TV','RDP','RDP-TI','NPG')
            title('Error Evolution (Iteration)')
            xlabel('Iteration'); ylabel('RMS Error')
        subplot(2,1,2)
            ...
            plot(cputimeSPIRALrdp,reconerrorSPIRALrdp,'m')
            plot(cputimeSPIRALrdpti,reconerrorSPIRALrdpti,'g')
            hold on; plot(npg003{1}.time,npg003{1}.RMSE,'k');
            legend('ONB','TV','RDP','RDP-TI','NPG')
            title('Error Evolution (CPU Time)')
            xlabel('CPU Time'); ylabel('RMS Error')
