...
%   Out.step - vector of norm(x - xp)/norm(x)
...
%   Out.time - cpu time at each iterations
...
Out.step = []; Out.itrs = []; Out.f = []; Out.lam = []; Out.cost=[];
Out.itr = Inf; oitr = 0;
...
for i = 1:opts.mxitr
    ...
    Out.cost=[Out.cost; f/(mu*mu_scale)];
    ...
    Out.step = [Out.step; nrmxxp/norm(x)]; 
