...
/*mex -v mexFunctions/source/proxSpMat2x2.c
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  double* X= mxGetPr(prhs[0]); // matrix input of size 6 x nx x ny x nz
  int  number_of_dims=mxGetNumberOfDimensions(prhs[0]);
  const mwSize *dims=mxGetDimensions(prhs[0]);
  
  if (dims[number_of_dims-1]!=3)
    mexErrMsgTxt("The last dimension of the input matrix should be equal to 3.\n");
  ...
  if(p==2){
    ...
    for(i=0; i < num_of_mat; i++){
      ...
      if (num_of_rho==1)
        proxS2(tmp2, tmp, rho[0]);
      else
        proxS2(tmp2, tmp, rho[i]);
 ...
 #pragma omp parallel for shared(X, Xp) private(i,k,tmp,tmp2)
    for(i=0; i < num_of_mat; i++){
      ...
      if (num_of_rho==1)
        proxSp(tmp2, tmp, rho[0], p, c0);
      else
        proxSp(tmp2, tmp, rho[i], p, c0);
