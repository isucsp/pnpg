...
if isempty(arg.we) | sum(abs(arg.we(:))) == 0  % no field map
  ...
  if ~isempty(arg.sens)
    arg.a0G = Gmri_SENSE(ktmp,logical(ones(N)),'fov',fov,'basis',{'dirac'},'nufft',arg.nufft_args, ...
                         'exact',0,'sens',arg.sens*(-1i*gam*arg.dt/2))';
    arg.b0G = Gmri_SENSE(ktmp,logical(ones(N)),'fov',fov,'basis',{'dirac'},'nufft',arg.nufft_args, ...
                         'exact',0,'sens',arg.sens*(1i*gam*arg.dt/2))';
  else
    arg.a0G = Gmri_SENSE(ktmp,logical(ones(N)),'fov',fov,'basis',{'dirac'},'nufft',arg.nufft_args, ...
