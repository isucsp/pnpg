...
%|					'mex', then 'imfilter', otherwise 'ind'
...
%|					(fast; needs image processing toolbox)
...
% default: try mex if available, if not, then 'imfilter', else 'ind'
...
if streq(arg.type_diff, 'imfilter') && exist('imfilter') ~= 2
	warn 'no imfilter (no image processing toolbox) so using slow convn'
	arg.type_diff = 'convn';
end
...
case {'convn', 'imfilter'}
	...
	ob = Fatrix(arg.dim, arg, 'caller', ['Cdiff1:' arg.type_diff], ...
		'forw', @Cdiff1_filt_forw, 'back', @Cdiff1_filt_back, ...
		'power', @Cdiff1_power, 'abs', @Cdiff1_abs);

case 'for1'
	ob = Fatrix(arg.dim, arg, 'caller', 'Cdiff1:for1', ...
		'forw', @Cdiff1_for1_forw, 'back', @Cdiff1_ind_back, ... % todo
		'power', @Cdiff1_power, 'abs', @Cdiff1_abs);
...
% Cdiff1_for1_loop1()
% a small function that i hoped matlab would compile "just in time"
...
if arg.order == 1
	...
	if 1
		for ll = 1:ncol(x)
			y(:,ll) = Cdiff1_for1_loop1(x(:,ll), ...
				1+max(off,0), arg.nn+min(off,0), off, coef);
		end
	else
		for ii = (1+max(off,0)):(arg.nn+min(off,0))
...
%
function y = Cdiff1_filt_forw(arg, x)
...
% x and y are both either [*N (L)] or [(N) (L)]
%
function x = Cdiff1_filt_back(arg, y)
arg.coef = flipdims(arg.coef); % trick
x = Cdiff1_filt_forw(arg, y);
