...
public class Histogram {
	 ...
	 */
	public void parseGrayHistogram(BufferedImage img){
	 ...
	 */
	public void parseHistogramForColors(BufferedImage img) {
		int height = img.getHeight();
		int width = img.getWidth();		
	...
	public static int[] normalize(int height, int[] histogram) {
		...
		ratio = ((double)max)/height;
		for (int i = 0; i < res.length; i++) {
			res[i] = (int) (histogram[i] / ratio);
		}
